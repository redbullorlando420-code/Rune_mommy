...
export function generateWorld() {
  ...
  for (let x = 0; x < W; x++) {
    block[0 * W + x] = 1; block[1 * W + x] = 1;
    block[(H - 1) * W + x] = 1; block[(H - 2) * W + x] = 1;
  }
  for (let y = 0; y < H; y++) {
    block[y * W + 0] = 1; block[y * W + (W - 1)] = 1; block[y * W + (W - 2)] = 1;
  }
  const trees = []; const rocks = []; const lights = []; const props = []; const buildings = [];
  ...
  for (let i = 0; i < 90; i++) {
    const x = 48 + (rng() * 20) | 0; const y = 6 + (rng() * 22) | 0;
    if (!canTree(x, y)) continue;
    block[y * W + x] = 1;
    trees.push({ x, y, variant: (rng() * 3) | 0, hue: 120 + rng() * 40 });
  }
  for (let i = 0; i < 20; i++) {
    const x = 10 + (rng() * 30) | 0; const y = 26 + (rng() * 10) | 0;
    if (!canTree(x, y)) continue;
    block[y * W + x] = 1;
    trees.push({ x, y, variant: 3, hue: 25 + rng() * 20 });
  }
  for (let i = 0; i < 16; i++) {
    const x = 56 + (rng() * 12) | 0; const y = 44 + (rng() * 8) | 0;
  ...
  function hutWalls(x, y, w, h, pal) {
    ...
    for (let yy = y; yy < y + h; yy++) {
      block[yy * W + x] = 1; block[yy * W + (x + w - 1)] = 1;
    }
  ...
  for (const s of SHOP_LAYOUT) {
    ...
    buildings.push({ id: s.id, kind: 'shop', x: s.x, y: s.y, w: s.w, h: s.h, hgt: s.hgt, palette: s.palette, flagship: !!s.flagship });
    npcs.push({ type: s.id, x: s.x + s.w / 2, y: s.y + s.h + 0.55 });
    lights.push({ x: s.x + s.w / 2, y: s.y + s.h / 2, r: s.flagship ? 6 : 3.4, color: s.flagship ? [255, 80, 216] : [255, 170, 80], flicker: s.flagship ? 1.1 : 0.5 });
  }
  buildings.push({ kind: 'mine', x: 58, y: 46, w: 8, h: 5, hgt: 2.4, palette: ['#4a4e56', '#2a2c30', '#8a8a9a'] });
  for (let i = 0; i < 18; i++) {
    const x = 12 + (rng() * 28) | 0; const y = 26 + (rng() * 10) | 0;
    if (block[y * W + x]) continue;
    const t = tiles[y * W + x];
  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      if (!TILE_WALK[tiles[y * W + x]]) block[y * W + x] = 1;
    }
  }
  ...
  const nodes = [
    { type: 'emberwood_tree', x: 54.5, y: 9.5 }, { type: 'emberwood_tree', x: 61.5, y: 11.5 },
    { type: 'emberwood_tree', x: 64.2, y: 14.4 }, { type: 'emberwood_tree', x: 58.8, y: 24.2 },
    { type: 'emberwood_tree', x: 50.5, y: 26.6 }, { type: 'moonbloom_patch', x: 63.2, y: 18.5 },
    { type: 'moonbloom_patch', x: 48.4, y: 20.8 }, { type: 'moonbloom_patch', x: 12.5, y: 14.5 },
    { type: 'moon_berry_bush', x: 18.2, y: 28.4 }, { type: 'moon_berry_bush', x: 32.5, y: 30.2 },
    { type: 'wild_mint_patch', x: 10.5, y: 34.2 }, { type: 'iron_outcrop', x: 60.5, y: 48.5 },
    { type: 'iron_outcrop', x: 64.2, y: 46.8 }, { type: 'iron_outcrop', x: 58.4, y: 50.4 },
    { type: 'iron_outcrop', x: 36.5, y: 34.5 },
  ];
  const mobs = [
    { type: 'gloom_wolf', x: 26.5, y: 30.5 }, { type: 'gloom_wolf', x: 30.5, y: 33.2 },
    { type: 'gloom_wolf', x: 22.2, y: 34.4 }, { type: 'gloom_wolf', x: 34.8, y: 31.8 },
    { type: 'ash_imp', x: 16.5, y: 29.5 }, { type: 'ash_imp', x: 14.2, y: 32.1 },
    { type: 'ash_imp', x: 19.1, y: 31.8 }, { type: 'dust_raccoon', x: 40.2, y: 28.5 },
    { type: 'dust_raccoon', x: 8.5, y: 26.4 }, { type: 'bramble_boar', x: 44.5, y: 34.2 },
    { type: 'bramble_boar', x: 50.2, y: 38.5 }, { type: 'hollow_stag', x: 66.2, y: 10.5 },
  ];
  return {
    w: W, h: H, tiles, walk, trees, rocks, lights, props, buildings, pois, npcs, nodes, mobs,
    zone: { id: 'clermont', name: 'Clermont Square', blurb: 'Hwy 50 shake row to the north. Johns Lake south. Emberfen and the mine to the east.' },
  };
}
