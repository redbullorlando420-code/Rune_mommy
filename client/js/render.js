...
export function createRenderer(canvas, mini) {
  ...
  function drawProps(state) {
    for (const p of state.props || []) {
      if (p.kind !== 'hut' && p.kind !== 'house') continue;
  ...
  function drawEntity(e, t) {
    ...
    if (e.kind === 'node') {
      ...
      if ((e.type || '').includes('tree') || e.type === 'emberwood_tree') {
        ctx.fillStyle = spent ? '#3a2a18' : '#c45c18';
        ctx.fillRect(x - 3, y - 6, 6, 14);
        ctx.fillStyle = spent ? '#2a2418' : '#6a994e';
        ctx.beginPath();
        ctx.arc(x, y - 14, 12, 0, Math.PI * 2);
        ctx.fill();
      } else if ((e.type || '').includes('iron') || e.type === 'iron_outcrop') {
