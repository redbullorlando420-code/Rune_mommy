...
import { loadCatalog, item, recipes } from '/shared/catalog.js';
import { NAME_RE, xpToLevel, INV_SIZE, BANK_SIZE } from '/shared/constants.js';
import { connect, send, on } from './net/client.js';
import * as W3 from './engine/three-world.js';

const state = {
  you: null, me: null, players: [], entities: [], tiles: null, w: 0, h: 0,
  trees: [], rocks: [], lights: [], props: [], buildings: [], pois: [],
  dest: null, selected: null, shopOpen: false, bankOpen: false,
};
let running = false;
...
W3.init(canvas);
window.addEventListener('resize', W3.resize);
function $(id) { return document.getElementById(id); }
function toast(text, kind) {
  const host = $('toasts'); if (!host) return;
  const n = document.createElement('div');
  n.className = 'toast ' + (kind || 'info'); n.textContent = text;
  host.appendChild(n); setTimeout(() => n.remove(), 3200);
}
function show(id, on) {
...
$('joinForm').addEventListener('submit', async (e) => {
  ...
  if (!NAME_RE.test(name)) { err.textContent = 'Name: start with a letter, 2-16 characters.'; return; }
  localStorage.setItem('rm-name', name);
  try { await connect(); } catch { err.textContent = 'Could not reach the server.'; return; }
  send(C2S.JOIN, { name });
});
...
on(S2C.WELCOME, (p) => {
  ...
  state.you = p.you; state.me = p.me; state.w = p.w; state.h = p.h; state.tiles = p.tiles;
  state.trees = p.trees || []; state.rocks = p.rocks || []; state.lights = p.lights || [];
  state.props = p.props || []; state.buildings = p.buildings || []; state.pois = p.pois || [];
  state.players = p.players || []; state.entities = p.entities || [];
  if (p.zone?.name && $('zoneName')) $('zoneName').textContent = p.zone.name;
  fillPois(); fillRecipes(); renderInv(); renderHud();
  running = true; W3.resize(); requestAnimationFrame(loop);
  pushChat({ channel: 'system', from: 'Hollow', text: p.zone?.blurb || 'Welcome to Clermont.' });
});
on(S2C.DELTA, (p) => {
  if (p.players) state.players = p.players;
  if (p.entities) state.entities = p.entities;
...
on(S2C.INV, (p) => {
  if (state.me) { if (p.inv) state.me.inv = p.inv; if (p.equip) state.me.equip = p.equip; }
  renderInv(); renderHud();
});
on(S2C.BANK, (p) => { if (state.me) state.me.bank = p.items || p.bank; renderBank(); });
on(S2C.XP, (p) => {
  if (state.me) {
...
on(S2C.NPC, (p) => {
  ...
  const acts = $('npcActions'); acts.innerHTML = '';
  if (p.shop) { const b = document.createElement('button'); b.className = 'gold'; b.textContent = 'Shop'; b.onclick = () => show('panelShop', true); acts.appendChild(b); }
  if (p.bank) { const b = document.createElement('button'); b.textContent = 'Vault'; b.onclick = () => { show('panelBank', true); renderBank(); }; acts.appendChild(b); }
});
on(S2C.SHOP, (p) => {
  state.shopOpen = true; show('panelShop', true);
  if (p.name) $('shopTitle').textContent = p.name;
  const box = $('shopBuy'); box.innerHTML = '';
...
function loop() {
  if (!running) return;
  const me = state.players.find((p) => p.id === state.you) || state.me;
...
canvas.addEventListener('pointerup', (e) => {
  ...
  const hit = W3.pickEntity(e);
  if (hit) {
    state.selected = hit.id;
    if (hit.kind === 'player' && hit.id !== state.you) { showCtx(e.clientX, e.clientY, hit); return; }
    if (hit.kind === 'mob') { send(C2S.ATTACK, { id: hit.id }); return; }
    send(C2S.INTERACT, { id: hit.id }); return;
  }
  const g = W3.pick(e);
  if (g) send(C2S.MOVE, { x: g.x, y: g.y });
});
...
function pushChat(p) {
  const ol = $('chatLog'); const li = document.createElement('li');
  const ch = p.channel || 'global'; li.className = ch;
...
function renderHud() {
  ...
  if ($('online')) $('online').textContent = String((state.players || []).length);
}
function slotEl(stack, i, kind) {
  const d = document.createElement('button');
  d.className = 'slot' + (stack ? '' : ' empty'); d.dataset.i = i; d.dataset.kind = kind;
  if (stack) {
    ...
    if (stack.qty > 1) { const q = document.createElement('span'); q.className = 'qty'; q.textContent = stack.qty; d.appendChild(q); }
  }
...
function onSlot(kind, i, shift) {
  if (kind === 'inv') {
    if ($('trade') && !$('trade').classList.contains('hidden')) { send(C2S.TRADE_SET, { slot: i, qty: 0 }); return; }
    if (state.shopOpen) { send(C2S.SHOP_SELL, { slot: i, qty: 0 }); return; }
    if ($('panelBank') && !$('panelBank').classList.contains('hidden') && shift) { send(C2S.BANK_PUT, { slot: i, qty: 0 }); return; }
    if (shift) send(C2S.DROP, { slot: i, qty: 0 }); else send(C2S.USE, { slot: i });
  }
  if (kind === 'equip') send(C2S.UNEQUIP, { slot: ['weapon', 'armor', 'charm'][i] });
  if (kind === 'bank') send(C2S.BANK_GET, { slot: i, qty: 0 });
  if (kind === 'tradeYou') send(C2S.TRADE_SET, { back: i });
}
function renderInv() {
  const me = state.me; if (!me) return;
  const grid = $('invGrid'); const eq = $('equipSlots'); if (!grid) return;
  grid.innerHTML = '';
  const inv = me.inv || [];
  ...
  if (eq) {
    eq.innerHTML = '';
    const e = me.equip || {};
    ['weapon', 'armor', 'charm'].forEach((s, i) => eq.appendChild(slotEl(e[s] ? { id: e[s], qty: 1 } : null, i, 'equip')));
  }
}
function renderBank() {
  const grid = $('bankGrid'); if (!grid || !state.me) return;
  grid.innerHTML = '';
  const bank = state.me.bank || [];
  for (let i = 0; i < BANK_SIZE; i++) grid.appendChild(slotEl(bank[i], i, 'bank'));
}
function fillPois() {
  const ul = $('poiList'); if (!ul) return; ul.innerHTML = '';
  for (const p of state.pois || []) {
    const li = document.createElement('li'); li.textContent = p.name;
    li.onclick = () => send(C2S.MOVE, { x: p.x, y: p.y }); ul.appendChild(li);
  }
}
function fillRecipes() {
  const box = $('recipeList'); if (!box) return; box.innerHTML = '';
  for (const rec of Object.values(recipes() || {})) {
    const b = document.createElement('button'); b.textContent = rec.name;
    b.onclick = () => send(C2S.CRAFT, { id: rec.id }); box.appendChild(b);
  }
}
function renderMinimap() {
  const c = $('minimap'); if (!c || !state.tiles) return;
  const ctx = c.getContext('2d'); const w = state.w, h = state.h;
...
function onTrade(p) {
  ...
  if (p.phase === 'cancel') { show('trade', false); show('tradeAsk', false); return; }
  show('trade', true); show('panelInv', true);
...
function showCtx(x, y, hit) {
  const ctx = $('ctx'); ctx.classList.remove('hidden');
  ctx.style.left = x + 'px'; ctx.style.top = y + 'px'; ctx.innerHTML = '';
