...
def make_humanoid(Entity, color, x, z, shirt, pants, skin=None, hitbox=False, detail='crowd', style=None):
    ...
    if style == 'michelle':
        ...
        # blonde head
        head = Entity(parent=root, model='sphere', color=_rgb(color, 255, 220, 170), scale=0.34, y=1.42)
...
def make_car(Entity, color, pos, yaw, paint):
    ...
    # cabin (kept as child so damage tint can reach it via car.cabin)
    ...
    # wheels — thin cubes (Ursina may lack built-in cylinder)
