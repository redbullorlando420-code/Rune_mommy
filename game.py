...
def data_report():
    ...
    portrait = ROOT / 'client/portraits/mira-coffee.jpg'
    return {
...
class Game:
    ...
    def _interact(self):
        ...
        if npc and nd <= 3.0 and (car is None or nd <= cd + 0.15):
            ...
            if kind == 'gun':
                ...
                return
            if kind == 'talk':
                self.open_talk(getattr(npc, 'npc_name', 'NPC'), getattr(npc, 'line', '...'))
                return
    ...
    def open_panel(self, title, body, choices, portrait=False):
        ...
        if portrait and self.mira_texture:
    ...
    def open_talk(self, name, line):
        self.open_panel(name, line, [('Later.', self.close_panel)])
    ...
    def _buy_item(self, item_id, price, name, stall):
        if self.gold < price:
            ...
            return
        self.gold -= price
        self.toast(f'{name}. Cold cup, Clermont dusk.')
        self.open_shake_shop(stall)
